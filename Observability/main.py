...
dotenv.load_dotenv(dotenv_path="D:\\Kshitij\\MAF\\V1\\Observability\\.env")
...
async def main():
    async with (
        ...
        with get_tracer().start_as_current_span("Single Agent Chat", kind=SpanKind.CLIENT) as current_span:
            ...
            agent = AzureAIClient(credential=credential, endpoint=os.environ["AZURE_AI_PROJECT_ENDPOINT"]).create_agent()
